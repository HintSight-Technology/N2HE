...
const int nh2 = 1;											// #Nodes in Hidden Layer

double w1[2048][nh2];                     // (DOUBLE) Weights between Input Layer and Hidden Layer
...
int ww1[2048][nh2];												// (int) Weights between Input Layer and Hidden Layer
...
void read_vector() {
	ifstream infile;
	infile.open("../ObjectClassification_data/test.txt");
	if(!infile.is_open()){
...
void read_w() {
	...
	ifstream infile;
	// infile.open("input/m1.txt");
	infile.open("../ObjectClassification_data/9898.txt"); 
	if(!infile.is_open()){
    	cout <<"Cannot open file 9898.txt"<<endl;
  	}
	// bias (1), weight (2048)
	double round_w = 1.0;
	//infile >> nh2;
...
int main(){
	...
	cout <<"Task: Privacy-preserving object classification"<<endl;
	int start_index = 0;
	int test_count = 1000;
	...
	// =========================== Start Testing ===============================================
	cout <<"Test start from image No. "<<start_index <<", to No. "<< start_index+test_count<<endl;
	int ans = 0;
	int err = 0;
	for (int tt = start_index; tt < start_index + test_count; ++tt) {
		...
		int dec = 0;
		for (int i = 0; i < 1; ++i) {
			dec = LWE32_Dec(q1, n1, ct_ip1[i], x);
		}

		if ((dec <= 0) && (tt < 500)) {
			ans ++;
		}
		else if ((dec > 0) && (tt >= 500)) {
			ans ++;
		}
		else;
	...
	// =========================== End of Testing ==============================================
	cout << "Correctness: " << ((double)ans/test_count)*100<<"%" << endl;
